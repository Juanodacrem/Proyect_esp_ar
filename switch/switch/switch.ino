...
void setup() {
  ...
  if (!display.begin()) {
    Serial.println(F("Error: No se pudo inicializar la pantalla Sharp."));
    while (1);  
  }
  Serial.println(F("Pantalla Sharp inicializada correctamente."));
  ...
  if (esp_now_init() != ESP_OK) {
    Serial.println(F("Error: No se pudo inicializar ESP-NOW."));
    return;
  }
  Serial.println(F("ESP-NOW inicializado correctamente."));
...
void handleButton() {
  ...
  if (is_long_press) {
    ...
    if (device_state) {
      send_command_to_mac("ENCENDIDO", current_menu_index);
      displayMessage("Encendido");
    } else {
      send_command_to_mac("APAGADO", current_menu_index);
      displayMessage("Apagado");
    }
  }
}
